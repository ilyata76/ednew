...
def closer_city(lat, lon, city1, city2):
    ...
    'Лондон'
    """
    return get_name(city1 if distance(make_city('current', lat, lon), city1) < distance(make_city('current', lat, lon), city2) else city2)
...
def put_piece(board, max_rows, column, player):
    ...
    def fulled(board, row, column) :
        return board[row][column] == 'O' or board[row][column] == 'X'
    
    new_row, new_board = -1, board

    if not fulled(board, 0, column):
        
        cur_row = 0
        not_fulled_index = cur_row

        while cur_row <= max_rows - 1:
            if not fulled(board, cur_row, column) :
                not_fulled_index = cur_row # запоминаем индекс незаполненной строки
            
            cur_row += 1

        new_row = replace_elem(board[not_fulled_index], column, player) # заменяем во вложенном списке элемент '-'
        new_board = replace_elem(board, not_fulled_index, new_row) # заменяем уже сам вложенный список на новый
        new_row = not_fulled_index

    return new_row, new_board
...
def print_board(board, max_rows, max_cols):
    ...
    while int_i < max_rows :
        while int_j < max_cols :
            lst.append(get_piece(board, int_i, int_j))
            int_j += 1
        int_i += 1
        int_j = 0; print(*lst); lst.clear()
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    lst, int_j = [], 0        
    while int_j < max_cols :
        lst.append(get_piece(board, row, int_j))
        int_j += 1

    Wlst, int_j = 0, 0
    
    while int_j < len(lst) :
        if lst[int_j] == player and Wlst < num_connect - 1:
            Wlst += 1
        elif lst[int_j] == player and Wlst == num_connect - 1:
            Wlst += 1
            break
        else :
            Wlst = 0
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    while int_j < max_rows :
        lst.append(get_piece(board, int_j, col))
        int_j += 1
