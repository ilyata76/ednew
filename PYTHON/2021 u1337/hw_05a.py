...
def replace_leaf(t, old, new):
    ...
    True
    """
    def get_tree(tree, old, new) :
        if is_leaf(tree) and label(tree) == old :
            return [new]
        elif is_leaf(tree) and label(tree) != old :
            return [label(tree)]
        else :
            return [label(tree)] + [get_tree(branch, old, new) for branch in branches(tree)]

    return get_tree(t, old, new)
...
def balanced(m):
    ...
    False
    """
    if not is_mobile(end(right(m))) and is_mobile(end(left(m))) :
        return length(right(m)) * size(end(right(m))) == length(left(m)) * total_weight(end(left(m))) and balanced(end(left(m)))
    elif is_mobile(end(right(m))) and not is_mobile(end(left(m))) :
        return balanced(end(right(m))) and length(left(m)) * size(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
    elif not is_mobile(end(right(m))) and not is_mobile(end(left(m))) :
        return length(right(m)) * size(end(right(m))) == length(left(m)) * size(end(left(m)))
    else :
        return balanced(end(right(m))) and balanced(end(left(m))) and (length(right(m)) * total_weight(end(right(m))) ==  length(left(m)) * total_weight(end(left(m))))
...
def totals_tree(m):
    ...
    if is_weight(m) :
        ...
        a, b = end(left(m)), end(right(m))

    return tree(total_weight(m), branches= [totals_tree(x) for x in [a, b] if x != None])
