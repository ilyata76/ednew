...
class Player(object):
    ...
    def play(self, card_index):
        ...
        2
        """
        assert len(self.hand) > 0 and card_index < len(self.hand), 'Неправильный индекс или в руке нет карт'
...
class BoyCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        True
        """
        int_j, int_numbers = 0, 3
        while int_j < int_numbers :
            opponent.hand.pop(0)
            opponent.draw()
            int_j += 1
...
class TeacherCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        a, d = other_card.attack, other_card.defense

        copy_lst_deck, int_j = player.deck.cards, 0
        while int_j < len(copy_lst_deck) :
            copy_lst_deck[int_j].attack += a
            copy_lst_deck[int_j].defense += d
            int_j += 1
        player.deck.cards = copy_lst_deck

        # исключаем карты
        opponent.deck.cards = [x for x in opponent.deck.cards if (x.attack != a and x.defense != d)]
